...
CUSTOM_SCHEME_EDGES = [
    ("A", "C", 4), ("B", "D", 1), ("B", "E", 5), ("C", "F", 2), ("D", "F", 2), ("E", "G", 3), ("F", "G", 3), ("G", "H", 4),
]
CUSTOM_POS = {"A": (0, 1.2), "B": (0, -1.2), "C": (1, 1.2), "D": (1, -0.5), "E": (1, -1.2), "F": (2, 0.4), "G": (2.5, -0.4), "H": (3.5, 0)}


def draw_variant17_scheme():
    """Схема варианта 17: A–B, A–C, B–D, C–E, D–F, E–F; критический путь A→B→D→F выделен красным."""
    G = nx.Graph()
    ...
    ax.set_title("Граф по варианту 17. Начало (A) — зелёная рамка, окончание (F) — оранжевая. Критический путь A→B→D→F — красный.", fontsize=STYLE["font_size"])
    ax.axis("off")
    plt.tight_layout()
    plt.savefig(os.path.join(IMAGES_DIR, "graph_variant17.png"), dpi=150, bbox_inches="tight")
    plt.close()
    print("Сохранено:", os.path.join(IMAGES_DIR, "graph_variant17.png"))
...
def draw_custom_scheme():
    ...
    # Критический путь по узлам (1,3,6,7,8) → (A,C,F,G,H)
    ...
    ax.set_title(f"Собственный граф. Начала (A, B) — зелёная рамка, окончание (H) — оранжевая. Критический путь: {path_str} — красный.", fontsize=STYLE["font_size"])
    ax.axis("off")
    plt.tight_layout()
    path = os.path.join(IMAGES_DIR, "graph_custom.png")
