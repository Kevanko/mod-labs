...
def add_graph_section(doc, jobs, figure_num, table_num, title, image_filename, description, steps_lines, figure_caption, table_caption):
    ...
    paragraph_no_indent(doc, "Расчёт выполнен по стандартному алгоритму: прямой ход (от старта к финишу) даёт ранние сроки событий РС и для каждой работы РН, РО = РН + t; обратный ход (от финиша к началу) даёт поздние сроки ПС и для работ ПН, ПО; затем резервы R и r; критический путь — работы с R = 0.")
    res = run_cpm(jobs)
    ...
    step4_text = (
        "Шаг 4. Резервы времени. Полный резерв R = ПО − РО (или R = ПН − РН; оба способа эквивалентны) — на сколько можно сдвинуть работу, не увеличивая срок проекта. "
...
def main():
    ...
    paragraph_no_indent(
        doc,
        "Прямой ход (от начала проекта к концу): для каждого события j ранний срок РС(j) — максимум из (РС предшественника + длительность входящей работы). По РС для каждой работы считают РН = РС(i) и РО = РН + t (раннее окончание получается в прямом ходе, не от финиша). Критическое время Tкр — ранний срок конечного события."
    )
    paragraph_no_indent(
        doc,
        "Обратный ход (от финиша к началу): только для поздних сроков — ПС(i) как минимум из (ПС преемника − длительность выходящей работы), затем для работ ПН и ПО. Для каждой работы: РН — раннее начало, РО — раннее окончание, ПН — позднее начало, ПО — позднее окончание. Полный резерв R = ПО − РО (или R = ПН − РН; оба способа эквивалентны); частный резерв r = РС(j) − РО. Критический путь составляют работы с R = 0."
    )
    ...
    # Собственный граф с одним истоком (событие 1): A,B из 1; C(A,4), D(B,1), E(B,5), F(D,C,2), G(E,F,3), H(G,4); Tкр=17
    desc_custom = (
        "Собственный граф с одним начальным событием: из 1 выходят работы A и B; далее C(A,4), D(B,1), E(B,5), F(D,C,2), G(E,F,3), H(G,4). "
        "Критический путь на схеме выделен красным (по событиям: A → B → E → F → G → H)."
    )
    steps_custom = [
        "Шаг 1. Прямой ход. РС(1) = 0. РС(2): A, 4. РС(3): B, 1. РС(5): C и D; max(4+4, 1+1) = 8. РС(6): E и F; max(1+5, 8+2) = 10. РС(7): G, 10+3 = 13. РС(8): H, 13+4 = 17. Tкр = 17.",
        "Шаг 2. Обратный ход. ПС(8) = 17, ПС(7) = 13, ПС(6) = 10, ПС(5) = 8, ПС(3) = min(8−1, 10−5) = 5, ПС(2) = 4, ПС(1) = 0.",
        "Шаг 3. Для каждой работы: РН = РС(i), РО = РН + t, ПО = ПС(j), ПН = ПО − t. Результаты в итоговой таблице ниже.",
        None,  # Шаг 4 генерируется из res в add_graph_section
        "Шаг 5. Критический путь — работы с R = 0: A, C, F, G, H; по событиям: A → B → E → F → G → H. Tкр = 17.",
    ]
    add_graph_section(
    ...
    paragraph_no_indent(doc, "2. Для собственного графа (8 работ, один исток) Tкр = " + str(res_custom["total_time"]) + "; критический путь: " + path_custom + ".")
